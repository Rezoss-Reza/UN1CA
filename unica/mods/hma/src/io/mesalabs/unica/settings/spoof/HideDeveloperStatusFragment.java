...
public final class HideDeveloperStatusFragment extends SettingsPreferenceFragment implements Preference.OnPreferenceClickListener {
    ...
    public void onCreate(Bundle state) {
        ...
        for(String key:new String[]{"enabled","targets","hidden","activity","invertActivity","apps","settings","risky","gids","logging","logs","viewLogs"}) findPreference("hma_"+key).setOnPreferenceClickListener(this);
        refresh();
    }
    ...
    private void refresh() {
        for(String key:new String[]{"enabled","activity","risky","logging"}) findPreference("hma_"+key).setSummary(config.optBoolean(key,"activity".equals(key)||"risky".equals(key)) ? "On" : "Off");
        for(String key:new String[]{"targets","hidden","invertActivity","apps","settings","gids","logs"}) findPreference("hma_"+key).setSummary(selected(key).size()+" selected");
    }
    private void toggle(String key) {
        try { config.put(key,!config.optBoolean(key,"activity".equals(key)||"risky".equals(key))); save(); }
        catch(JSONException ignored) {}
    }
    public boolean onPreferenceClick(Preference p) {
        String key=p.getKey().substring(4);
        if(Arrays.asList("enabled","activity","risky","logging").contains(key)) toggle(key);
    ...
    private void choices(final String key,String title,final String[] values,String[] labels, final Set<String> retained) {
        ...
        for(int i=0;i<values.length;i++) checked[i]=chosen.contains(values[i]);
        ...
        new Thread(new Runnable(){public void run(){
            try {
                ...
                for(ApplicationInfo a:pm.getInstalledApplications(0)) {
                    ...
                    if(!"hidden".equals(key) && (a.uid%100000<10000 || a.packageName.equals("com.android.settings") || a.packageName.equals("com.android.systemui"))) continue;
                    apps.add(a);
                }
                ...
                for(int i=0;i<apps.size();i++){ApplicationInfo a=apps.get(i); values[i]=a.packageName; labels[i]=pm.getApplicationLabel(a)+"\n"+a.packageName; retained.remove(a.packageName);}
                handler.post(new Runnable(){public void run(){if(isAdded()) choices(key,"hidden".equals(key)?"Additional apps to hide":"invertActivity".equals(key)?"Invert activity protection for":"Apps to protect",values,labels,retained);}});
